...
int main(void)
{
    WDTCTL = WDTPW | WDTHOLD;   // stop watchdog timer
            ...
            P5REN |= BUTTON1;       //Enables Pullup resistor on Button1
            P5OUT |= BUTTON1;       //Makes pullup resistor for button1

            P5IES &= BUTTON1;     //interrupts when button is pressed
            P5IE |= BUTTON1;       //enables interupt for button
            P5IFG &= ~BUTTON1;     //ands the interupt flag with 0 to clear it
...
__interrupt void TIMER0_A0_ISR(void) {
    ...
    P5IE |= BUTTON1; //enables interrupt
    P5IFG &= ~BUTTON1; //Resets Interrupt Flag
}
