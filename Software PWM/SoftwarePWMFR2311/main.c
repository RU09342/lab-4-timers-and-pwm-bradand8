...
#define LED_DIR2    P2DIR
#define LED_OUT1    P1OUT
#define LED_OUT2    P2OUT
#define BUTTON      BIT1
...
__interrupt void Port_1(void) {
    ...
    if (P1IN & BUTTON){   //if button is pressed
        ...
        TB0CCR1 = TB0CCR1 + 100;  //Duty cycle increases by 10%
    }
    else {
        LED_OUT2 |= LED2
...
void timerA1Setup(){
    ...
    TB0CCR1 |= 500; //PWM start 50% Duty cycle     .50 X10000K Hz freq = 500ticks led on  Duty cycle = TACCR1/ TACCR0 +1

    TB0CCTL1 |= OUTMOD_7; //counts up and then resets when hit TA0CCR0
    // SMCLK, Up Mode (Counts to TA0CCR0)
    TB0CTL |= TBSSEL_2 + MC_1 + ID_3; //1M hz clock upcount mode divided by 8
...
void timerA0Setup(){
    ...
    TB0CTL = TBSSEL_2 + MC_1 + ID_3;  //Uses 1000000Hz clock, divided by 8 in Up count mode
    TB0CCR0 = 1000;
